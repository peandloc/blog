...
class BaseController extends Controller {
    ...
    protected function _initialize(){
        ...
        //select table said
        //$saids = D('Dongtan')->order("d_id desc")->limit(5)->select();
        //$model      = new \Think\Model;
        //$table = ['web_dongtan'=>'d',"web_admin"=>'a'];
        //$saids = $model->table($table)->where('a.id = d.u_id')->order('d.d_id desc')->limit(5)->select();
        // foreach($saids as $key=>$val){
            // foreach($val as $k=>$v){
        // for($i=0;$i<count($saids);$i++){
        //     $saids[$i]['time'] = date('Y-m-d H:i:s',$saids[$i]['time']);
        // }
        $saids = D('dongtanView')->order('d_id desc')->limit(5)->select();
        foreach($saids as $key=>$val){
            $saids[$key]['time'] = date('Y-m-d H:i:s',$saids[$key]['time']);
        }
        $this -> assign('dongs',$saids);
        // if(session('?username')){
        //     $username = ['sign'=>1,'username'=>session('username')];
        // }
        $infologin = session('?username') ? ['islogin'=>'1','username'=>session('username'),'user'=>session('name')] : $username = ['islogin'=>'0'];
        $this -> assign('infologin',$infologin);
    }
